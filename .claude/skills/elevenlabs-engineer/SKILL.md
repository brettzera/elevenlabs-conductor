...
8. **Client memory lives in the client folder; the repo's own files are Brett-gated.** Alongside the feedback ledger, the client folder holds `<client-folder>/CLAUDE.md` — the client-journey memory (agent ids, build history, key decisions, current state, standing constraints). Read it at run start (create it on first contact) and update it at hand-off. This repo's house docs (`docs/workflow-patterns.md`, `docs/house-persona.md`, `docs/guardrails.md`, `docs/latency-playbook.md`, the create-agent conventions) are binding defaults — but you NEVER edit this repo's files (docs, skills, agents, hooks) or open a PR against it on your own initiative. If a fix's root cause points at a repo defect (a doc is wrong, a default keeps biting), write the proposal — what to change, why, and the evidence, with no client PII — to `<client-folder>/repo-change-proposals.md` and tell the operator to send it to Brett: he reviews, applies, and pushes repo changes himself. No answer means no.
...
- **Read the client-journey memory** (`<client-folder>/CLAUDE.md`) if it exists — its recorded decisions and standing constraints frame every fix; create it (client, agent, date, goal) if it doesn't.
...
- **Update the client-journey memory:** refresh `<client-folder>/CLAUDE.md` with what this run fixed and staged, the decisions made (and why), the current agent/branch state, and any new standing constraints — so the next session picks up the journey instead of rediscovering it. If any fix's root cause points at a repo defect (a house doc is wrong, a default keeps biting), write the proposal — what to change, why, evidence, no client PII — to `<client-folder>/repo-change-proposals.md` per Golden Rule 8 and remind the operator to send it to Brett; never edit this repo's files or open a PR yourself, and never let an unsent proposal shape a fix as if it were already approved.
...
- Client memory is per-client only: the client folder's `CLAUDE.md` (journey memory, updated at hand-off) and feedback ledger — nothing about a client ever lands in this repo. The repo's house docs are binding defaults, and its files are Brett-gated: proposed repo changes go in `<client-folder>/repo-change-proposals.md` for the operator to send to Brett, never direct edits or PRs.

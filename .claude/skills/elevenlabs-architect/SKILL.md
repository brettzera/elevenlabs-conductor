...
8. **Approved lessons are binding; house defaults change only with Brett's explicit approval.** Before Phase 4, read `## Approved house lessons` in [docs/cross-client-lessons.md](../../../docs/cross-client-lessons.md) and treat every entry as a house default — fold the relevant ones into the configurator dispatch briefs. The `## Candidate observations` in that file are signals only and must NOT influence any build decision. If this run surfaces a generalizable pattern worth becoming a house rule (e.g. the analyzer red-flags the same template default that already bit another client), you may append a candidate row (obeying that file's privacy rules) and — at or past its proposal threshold — raise the written proposal through that file's one channel: a **lesson pull request** (Brett's PR approval and merge is the approval — for everyone, Brett included; a chat "yes" never substitutes for the PR); you NEVER edit the Approved section, `latency-playbook.md`, template/agent defaults, or any skill's instructions outside a merged lesson PR. No answer means no.
...
- New agents: initial build on Main, built fresh from the create-agent reference's house defaults (HL-004 — never a MASTER TEMPLATE clone).
...
- Approved house lessons (`docs/cross-client-lessons.md`) are binding defaults; candidate observations are signals only. Never change a house default — Approved section, latency playbook, template/agent/skill defaults — without Brett's explicit chat approval of a written proposal.
